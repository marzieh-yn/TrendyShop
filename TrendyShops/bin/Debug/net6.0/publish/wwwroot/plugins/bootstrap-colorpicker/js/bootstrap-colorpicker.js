...
function Color(obj, model) {
	...
	if (model && model in skippedModel) {
		model = null;
	}
...
Object.keys(convert).forEach(function (model) {
	if (skippedModel.indexOf(model) !== -1) {
		return;
	}
...
var Model = Object.keys(conversions);
...
Model.forEach(function (fromModel) {
	convert[fromModel] = {};
	...
	var routeModel = Object.keys(routes);

	routeModel.forEach(function (toModel) {
...
function buildGraph() {
	...
	for (var len = Model.length, i = 0; i < len; i++) {
		graph[Model[i]] = {
